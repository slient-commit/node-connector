...
class CopyFileWindows extends Plugin {
  name() {
    return "Copy File (Windows)";
  }

  description() {
    return "Copy files or folders on a Windows system using xcopy/copy commands.";
  }
  ...
  async logic(params = {}) {
    ...
    let command;
    if (recursive) {
      command = `xcopy "${resolvedSource}" "${resolvedDest}" /E /I /Y /Q`;
    } else {
      command = `copy /Y "${resolvedSource}" "${resolvedDest}"`;
    }
...
module.exports = CopyFileWindows;
